...
type Service = { id: string; name: string; status: Condition; latencyMs?: number | null; checkedAt?: string | null; message?: string | null; uptime?: number | null; sampleCount?: number; monitoringSince?: string | null; history?: Array<{ at: string; status: Condition }> }
...
function parseStatus(input: unknown): StatusSnapshot | null {
  ...
  return {
    ...
    services: data.services.filter((item): item is Record<string, unknown> => !!item && typeof item === 'object').map((item) => ({ id: String(item.id || ''), name: String(item.name || item.id || ''), status: validCondition(item.status), latencyMs: typeof item.latencyMs === 'number' ? item.latencyMs : null, checkedAt: typeof item.checkedAt === 'string' ? item.checkedAt : null, message: typeof item.detail === 'string' ? item.detail : null, uptime: typeof item.uptimePercent === 'number' ? item.uptimePercent : null, sampleCount: typeof item.sampleCount === 'number' ? item.sampleCount : 0, monitoringSince: typeof item.monitoringSince === 'string' ? item.monitoringSince : null, history: Array.isArray(item.history) ? item.history as Service['history'] : undefined })),
